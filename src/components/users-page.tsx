...
const AutoGenerateDialog = ({ isOpen, onOpenChange, users }: { isOpen: boolean, onOpenChange: (open: boolean) => void, users: AppUser[] }) => {
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
                        ...
                        <TableBody>
                            {users.map(user => (
                                <TableRow key={user.id}>
                                    ...
                                    <TableCell className="text-xs text-muted-foreground truncate max-w-xs">{user.fcmToken || "Not Available"}</TableCell>
...
export default function UsersPage() {
    ...
    const openDetailsDialog = (user: AppUser, details: UserDetails | null) => {
        setSelectedUser({ user, details });
        setIsDetailsDialogOpen(true);
    }
    
    const visibleUsers = users.slice(0, visibleUsersCount);
    const hasMoreUsers = users.length > visibleUsersCount;
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            <header className="mb-6 flex justify-between items-center">
                ...
                <Button onClick={() => setIsAutoGenerateOpen(true)}>Auto-Generate</Button>
            </header>
            <Card>
                <CardHeader>
                ...
                <CardContent>
                            ...
                            {visibleUsers.map((user) => {
                                ...
                                return (
                                    <TableRow key={user.id}>
                                        ...
                                        <TableCell className="text-right">
                                            <Button variant="outline" size="sm" onClick={() => openDetailsDialog(user, details)}>View</Button>
                                        </TableCell>
                                    </TableRow>
                                );
                            })}
                    ...
                    {hasMoreUsers && (
                        <div className="pt-4 text-center">
                            <Button variant="outline" onClick={() => setVisibleUsersCount(prev => prev + ITEMS_PER_PAGE)}>
                                Load More
                            </Button>
                        </div>
                    )}
